...
import org.mockito.Matchers.{ any, same, eq => equalTo }
...
class TasksResourceTest extends MarathonSpec with Matchers {
  ...
  test("killTasks") {
    ...
    verify(taskKiller, times(1)).kill(equalTo(app1), any(), equalTo(false))
    verify(taskKiller, times(1)).kill(equalTo(app2), any(), equalTo(false))
